...
result_dir = './SonyA7S2_pure_train/'
...
lastepoch = 1600
learning_rate = 4e-5
use_dpvs = False # use deep supervision
...
if lastepoch == 0:
    ...
    if use_dpvs:
        model = load_weights(model, path=model_dir+'checkpoint_sonya7s2_dpvs_e%04d.pth'%lastepoch)
    else:
        model = load_weights(model, path=model_dir+'checkpoint_sonya7s2_pure_e%04d.pth'%lastepoch)
...
scheduler = optim.lr_scheduler.StepLR(optimizer, gamma=0.5, step_size=200)
    ...
    with tqdm(total=len(train_ids)) as t:
        for i, ind in enumerate(random_inds):
            ...
            for i in range(len(input_images)):
                input_patch[i] = generate_noisy_obs(input_patch[i], camera_type='SonyA7S2')
            # 图像增强（翻转，旋转会破坏行噪声假设）
        ...
        if epoch % plot_freq == 0:
            ...
            filename = result_dir + '%04d_%05d_sonya7s2_train.png'%(epoch,train_id)
            plt.imsave(filename, temp)
            # cv2.imwrite(filename, np.uint8(temp[...,::-1]*255))
        
        if epoch % save_freq == 0:
            log(f"learning_rate: {scheduler.get_last_lr()[0]:.6f}")
            torch.save(model.state_dict(), model_dir+'checkpoint_sonya7s2_pure_e%04d.pth'%epoch)
